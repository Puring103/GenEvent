...
### 2. 线程检查采用显式开关，而不是条件编译

线程检查使用公开运行时开关：

- `GenEventRuntimeSettings.EnableThreadAccessChecks`

不依赖 `DEBUG` 条件编译。这样测试更容易覆盖，宿主项目也能按需启用或关闭。
...
搭建初始化状态、线程归属和统一错误消息所需的基础设施，为后续发布/订阅入口改造提供公共支撑。
...
- 在 `Init()` 中调用统一的运行时线程归属初始化逻辑。

2. 新增运行时设置与守卫文件：

- `src/GenEvent/src/GenEventRuntimeSettings.cs`
- `src/GenEvent/src/Internal/GenEventRuntimeGuard.cs`
...
- 记录归属线程 ID；
- 提供统一检查方法，例如：
  - `EnsureInitialized(string operationName)`
  - `EnsurePublisherRegistered(Type eventType, string operationName)`
  - `EnsureThreadAccess(string operationName)`
- 统一构造 `InvalidOperationException` 消息，避免每个入口各写一套文案。
...
- 在 `Publish()` 和 `PublishAsync()` 入口先执行线程检查；
...
- 在 `StartListening()` / `StopListening()` 以及特定事件类型重载入口增加初始化检查与线程检查；
...
- `PublishConfig<TGenEvent>.Setting` 当前是静态可变状态，本阶段不修改其线程模型，只在入口上做契约检查；
...
- 默认关闭时不因检查逻辑而改变现有行为
- 开启后，跨线程 `Publish()` 失败
- 开启后，跨线程 `StartListening()` / `StopListening()` 失败
- 关闭后不应误报线程错误
...
- 线程相关测试要确保开关状态在每个用例后恢复，避免污染其他测试；
...
- 线程检查如果侵入位置选错，可能误伤正常单线程重入场景；
- 静态状态测试隔离不充分时，测试结果可能不稳定。

### 回滚点

如果实现过程中发现线程检查设计明显干扰现有调用模式，可保留：
...
并将线程检查从本次版本降级为延后项。这样仍然不影响本次版本的主要价值。
